...
for node in p:
    print(node.action)
    if(node.action != None):
        arquivo.write(node.action + "\n")
    node.puzzle.pprint()
    steps += 1

arquivo.write("Total de passos: " + str(steps))
arquivo.write("\n")
arquivo.write("Tempo gasto na pesquisa: " + str(toc - tic) + " em segundos")
